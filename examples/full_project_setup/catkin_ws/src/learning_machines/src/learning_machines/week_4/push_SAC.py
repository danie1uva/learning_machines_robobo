...
class PerformanceBasedRandomizationCallback(BaseCallback):
    ...
    """
    def __init__(self, env: CoppeliaSimEnv, verbose=0):
        ...
        self.stages = [
            ...
            {'success_rate': 81, 'frequency': 2},
            {'success_rate': 55, 'frequency': 2}  # Final stage to stop training
        ]

    def _on_step(self) -> bool:
        # Check if an episode has finished
        if 'success' in self.locals['infos'][0]:
            success = self.locals['infos'][0]['success']
...
def train_sac_dynamic_randomization(rob: IRobobo):
    ...
    wandb.init(
        ...
        config={
            ...
            "ent_coef": "auto",
            "max_grad_norm": 0.5,
            "total_timesteps": 200000,  # Initial total timesteps; may stop earlier based on performance
