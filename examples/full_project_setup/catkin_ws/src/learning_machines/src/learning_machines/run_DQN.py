...
def move_robobo_and_calc_reward(scaler, action, rob, state):
    ...
    if left_speed > 0 and right_speed > 0:
        forward_reward = 5 * min(left_speed, right_speed) / 100  # Encourage forward 
    elif left_speed < 0 and right_speed < 0:
        forward_reward = -30  # penalise backward motion
    else:
        forward_reward = 0
...
def run_qlearning_classification(rob: IRobobo):
    ...
    model_path = f"trained_qnetwork_{timestamp}.pth"
