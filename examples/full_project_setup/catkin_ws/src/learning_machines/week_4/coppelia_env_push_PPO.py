...
class CoppeliaSimEnv(gym.Env):
    def __init__(
        ...
        # Action space: left wheel, right wheel (0..100)
        ...
        # Observation: 5 IR sensors + bounding boxes for puck & green zone
    ...
    def step(self, action):
        ...
        print(f"Step {self.total_step_count}, Action: {left_wheel}, {right_wheel}")
        ...
        if self._front_collision(next_state):
            ...
            # Normal reward logic if no collision
        ...
        if self.steps_in_episode >= self.max_steps:
            self.done = True
            # Optionally add a small penalty if we want the agent to avoid timing out:
            # reward -= 1.0
    ...
    def _calculate_reward_and_done(self, puck_box):
        ...
        if self.stage == 1:
            ...
            if not self.puck_reached:
                ...
                proximity_reward = self._exponential_distance_reward(distance, alpha=2.0, min_dist=0.0)
                reward += proximity_reward
    ...
    def _front_collision(self, obs):
        """
        Check if front sensors read > 0.5 => collision with a wall.
        The first 5 elements of obs are IR sensors after normalisation.
        """
        front_sensors = obs[:5]
        # Adjust logic or indexing if your sensor arrangement differs
